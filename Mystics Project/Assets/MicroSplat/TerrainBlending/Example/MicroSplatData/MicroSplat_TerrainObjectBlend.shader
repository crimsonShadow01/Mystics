...
Shader "MicroSplat/MicroSplat_TerrainBlending_TerrainObjectBlend" {
   Properties {
...
UNITY_INSTANCING_BUFFER_END(Terrain)
      ...
      #if !_DEBUG_USE_TOPOLOGY && !_MICROMESHTERRAIN && !_TERRAINBLENDABLESHADER && !_MICROMESH && !_MICROMESHTERRAIN && !_MICROPOLARISMESH &&!_MICRODIGGERMESH && !_MICROVERTEXMESH && defined(UNITY_INSTANCING_ENABLED) && !defined(SHADER_API_D3D11_9X)
      
      float4 _TerrainHeightmapTexture_TexelSize;
      float4 SampleInstancedTerrainDesc(float uv)
      {
            float height = UnpackHeightmap(tex2D(_TerrainHeightmapTexture, uv));
            height *= _TerrainHeightmapScale.y; 
            float3 normal = normalize(tex2D(_TerrainNormalmapTexture, uv).xyz * 2 - 1);
      ...
      float4 SampleTerrainDesc(inout SurfaceOutputCustom s, out float normBlend)
      {
         ...
         #if !_DEBUG_USE_TOPOLOGY && !_MICROMESHTERRAIN && !_TERRAINBLENDABLESHADER && !_MICROMESH && !_MICROMESHTERRAIN && !_MICROPOLARISMESH &&!_MICRODIGGERMESH && !_MICROVERTEXMESH && defined(UNITY_INSTANCING_ENABLED) && !defined(SHADER_API_D3D11_9X)
            float4 texelSize = _TerrainHeightmapTexture_TexelSize;
         #else
            float4 texelSize = _TerrainDesc_TexelSize;
         #endif
