...
public class MicroSplatBlendableObjectEditor : Editor 
{
   ...
   public override void OnInspectorGUI()
   {
      ...
      var terrain = msObj.GetComponent<Terrain> ();
      if (msObj.terrainDesc == null && (terrain != null && terrain.drawInstanced == false)) 
      {
         EditorGUILayout.HelpBox("Terrain does not have blending data, please select the terrain and generate it.", MessageType.Error);
         return;
      }
      
      MicroSplatTerrain tr = msObj.GetComponent<MicroSplatTerrain>();
      if (tr != null && tr.sTerrainDirty)
      {
