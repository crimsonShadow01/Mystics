...
Shader "MicroSplat/Example_GlobalTexturing" {
   Properties {
   ...
   weights = half3(w1, w2, w3);
      ...
      void GeoTexturePerTex(inout RawSamples s, float3 worldPos, float worldHeight, Config config, float3 worldNormal, float3 upVec)
      {
            ...
            half4 strengths = half4(1,1,1,1);
               ...
               geoNorm.xy *= _GeoNormalStrength;
               s.normSAO0.xy = lerp(s.normSAO0.xy, BlendNormal2(s.normSAO0.xy, geoNorm.xy), geoStr * hblend.x * strengths.x);
               s.normSAO1.xy = lerp(s.normSAO1.xy, BlendNormal2(s.normSAO1.xy, geoNorm.xy), geoStr * hblend.y * strengths.y);
               #if !_MAX2LAYER
                  s.normSAO2.xy = lerp(s.normSAO2.xy, BlendNormal2(s.normSAO2.xy, geoNorm.xy), geoStr * hblend.z * strengths.z);
               #endif
               #if !_MAX2LAYER && !_MAX3LAYER
                  s.normSAO3.xy = lerp(s.normSAO3.xy, BlendNormal2(s.normSAO3.xy, geoNorm.xy), geoStr * hblend.w * strengths.w);
               #endif
            #endif
      ...
      void GeoTexture(inout half3 albedo, inout half4 normalSAO, float3 worldPos, float worldHeight, Config config, float3 worldNormal, float3 upVec)
      {
         #if !_PERTEXGEO && !_PERTEXGEOMAPHEIGHT
            float2 geoUV = float2(0, worldHeight * _GeoParams.y + _GeoParams.z);
               #if _GEOCURVE
            ...
            float geoStr = _GeoParams.x * geoTex.a;
               ...
               half3 geoNorm = UnpackNormal(UNITY_SAMPLE_TEX2D_SAMPLER(_GeoNormal, _Diffuse, geoUV));
               geoNorm.xy *= _GeoNormalStrength;
               normalSAO.xy = lerp(normalSAO.xy, BlendNormal2(geoNorm.xy, normalSAO.xy), geoStr);
            #endif
         #endif
      }
      ...
      {
         float2 uv = (config.uv + _GlobalTintUVScale.zw) * _GlobalTintUVScale.xy + noiseUV;
      ...
      void GlobalNormalTexturePerTex(inout RawSamples s, Config config, float camDist, float slopeFilter, float2 noiseUV)
      {
            ...
            half4 normalSAO = half4(0,0,0,1);
               ...
               normalSAO.xy = UnpackNormal(rawNormal);
      ...
      // no per tex is faster, just final value.
      void GlobalNormalTexture(inout half4 normSAO, Config config, float camDist, float slopeFilter, float2 noiseUV)
      {
         #if !_PERTEXGLOBALNORMALSTRENGTH
            float2 uv = (config.uv + _GlobalNormalUVScale.zw) * _GlobalNormalUVScale.xy + noiseUV;
            float str = _GlobalTextureParams.y;
            ...
            #if _GLOBALNORMALPACKEDSAO
               tex.xy = UnpackNormal(rawNormal);
               tex.zw = rawNormal.xz;
            #else
               tex.xy = UnpackNormal(rawNormal).xy;
            #endif
...
ENDCG
   ...
   Dependency "BaseMapShader" = "Hidden/MicroSplat/Example_GlobalTexturing_Base1870711408"
