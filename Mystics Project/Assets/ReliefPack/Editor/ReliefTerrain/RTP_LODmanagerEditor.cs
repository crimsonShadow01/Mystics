    ...
    private bool RebuildFeaturesInFile(string shader_path, bool shader_flag = true, bool AddPass_flag = true, bool geom_flag = false, bool blend_base = false)
    {
        ...
        if (System.IO.File.Exists(shader_path))
        {
            ...
            if (shader_flag)
            {
                ...
                {// || shader_path=="Assets/ReliefPack/Shaders/ReliefTerrain/ReliefTerrain2Geometry.shader") {
            ...
            // MaxLOD treatment
            if (mainshaders_flag)
            { // warunek, aby nie przebudowywac dodatkowych shaderow, tylko te uzywane prez teren i geom)
                sidx = 0;
                int occurence = 0;
                do
                {
