...
namespace MeshCombineStudio
{
    ...
    public class CreateOverlapColliders : MonoBehaviour
    {
        ...
        static public Transform newT;

        static int overlapLayer;
        static float colliderScaleMulti = 1;
        ...
        void Update()
        {
            if (create)
            {
                ...
                Create(transform, layerMask, 4, ref newGO, true);
            }
            if (destroy)
            {
                destroy = false;
        ...
        {
            var sceneColliders = Methods.SearchAllScenes<Collider>(false);

            int layerMaskValue = layerMask.value;

            for (int i = 0; i < sceneColliders.Count; i++)
            {
                Collider collider = sceneColliders.items[i];
        ...
        static public bool Create(Transform parentT, LayerMask overlapLayerMask, int lodGroupLayer, ref GameObject overlapCollidersGO, bool removeSamePositionTriangles)
        {
            ...
            if (layerMaskValue == 0) return false;
        ...
        static public void CopyTransform(Transform st, Transform dt)
        {
            ...
            dt.localScale = st.lossyScale * colliderScaleMulti;
        }
    }
}
